...
$confirmDelete = function ($id) {
    $this->id = $id;
    $this->dispatch('delete-news-confirmation');
};
...
<div>
                    ...
                    <div x-data="usePopper({placement:'bottom-end',offset:4})"
                        ...
                        <div x-ref="popperRoot" class="popper-root" :class="isShowPopper && 'show'">
                            <div
                                ...
                                <ul>
                                    <li><a wire:navigate.hover href="{{ route('admin.berita.create') }}"
                        ...
                        <tbody>
                            @foreach ($this->news as $index => $item)
                                <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                        <div
                                            class="badge space-x-2.5 rounded-full {{ $item->status === 'published' ? 'bg-success/10 text-success dark:bg-success/15' : 'bg-warning/10 text-warning dark:bg-warning/15' }}">
                                            <div
                                                class="size-2 rounded-full {{ $item->status === 'published' ? 'bg-current' : 'bg-current' }}">
                                            </div>
                                                ...
                                                <div
                                                    ...
                                                    <ul>
                                                        <li><a wire:navigate.hover
                                                                href="{{ route('admin.berita.edit', ['id' => $item->id]) }}"
    ...
    <x-confirm-modal :trigger="'delete-news-confirmation'" :title="'Konfirmasi Penghapusan'" :message="'Berita ini akan dihapus. Apakah Anda yakin ingin menghapus data ini?'" :action="'delete'" />

    <x-success-modal :trigger="'delete-news-confirmed'" :title="'Berita berhasil dihapus'" :message="'Data berita telah dihapus.'" />
