...
$confirmDelete = function ($id) {
    $this->id = $id;
    $this->dispatch('delete-gallery-confirmation');
};
...
<div>
                    ...
                    <div class="flex items-center" x-data="{isInputActive:false}">
                        <label class="block">
                            <input wire:model.live.debounce.250ms="search"
                                ...
                                placeholder="Cari Galeri..." type="text" />
                        </label>
                    ...
                    <div x-data="usePopper({placement:'bottom-end',offset:4})"
                        ...
                        <div x-ref="popperRoot" class="popper-root" :class="isShowPopper && 'show'">
                            <div
                                ...
                                <ul>
                                    <li><a wire:navigate.hover href="{{ route('admin.galeri.create') }}"
                        ...
                        <tbody>
                            @foreach ($this->galleries as $index => $item)
                                <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                                        <div
                                            class="badge space-x-2.5 rounded-full {{ $item->status === 'aktif' ? 'bg-success/10 text-success dark:bg-success/15' : 'bg-warning/10 text-warning dark:bg-warning/15' }}">
                                            <div
                                                class="size-2 rounded-full {{ $item->status === 'aktif' ? 'bg-current' : 'bg-current' }}">
                                            </div>
                                                ...
                                                <div
                                                    ...
                                                    <ul>
                                                        <li><a wire:navigate.hover
                                                                href="{{ route('admin.galeri.edit', ['id' => $item->id]) }}"
    ...
    <x-confirm-modal :trigger="'delete-gallery-confirmation'" :title="'Konfirmasi Penghapusan'" :message="'Galeri ini akan dihapus. Apakah Anda yakin ingin menghapus data ini?'" :action="'delete'" />

    <x-success-modal :trigger="'delete-gallery-confirmed'" :title="'Galeri berhasil dihapus'" :message="'Data galeri telah dihapus.'" />
