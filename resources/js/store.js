export default {
  ...
  init() {
    ...
    document.addEventListener('livewire:navigated', () => {
      ...
      // Recalculate based on current DOM and Breakpoints
      this.isSidebarExpanded =
        document.querySelector(".sidebar") &&
        document.body.classList.contains("is-sidebar-open") &&
        Alpine.store("breakpoints").xlAndUp;

      // CRITICAL FIX: Sync DOM on Mobile
      // If we are on mobile, force the class removal because Alpine might not trigger if state didn't change (False -> False)
      if (!Alpine.store("breakpoints").xlAndUp) {
